...
export function UpdateStatus({ state, autoUpdate, stale, intervalMs = config.pollIntervalMs, staleAfterMs = config.staleAfterMs }: { state: MonitorState; autoUpdate: boolean; stale: boolean; intervalMs?: number; staleAfterMs?: number }) {
  return <div className="update-status" role="status">
    <div><span className={'status-dot ' + (state.error || stale ? 'warning' : '')} /><strong>{labels[state.phase]}</strong>
      <span className="muted">{autoUpdate ? (intervalMs / 60_000) + '分ごとに確認' : '自動更新は停止中'}</span></div>
    <p>最終確認成功 <time>{formatTime(state.lastSuccessAt)}</time></p>
    {stale && <p className="warning-text">最新の取得データが{staleAfterMs / 60_000}分以上古くなっています。</p>}
